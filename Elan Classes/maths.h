...
public:
    ...
    inline bool is_inside(const RANGE & r) const { return r.start() >= s && r.end() <= e; }
    inline bool is_outside(const RANGE & r) const { return r.start() < s && r.end() > e; }
    inline bool is_before(const RANGE & r) const { return r.start() <= s && r.end() <= s; }
    inline bool is_after(const RANGE & r) const { return r.start() >= e && r.end() >= e; }
